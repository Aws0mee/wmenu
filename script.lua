...
function warped.sendlua(lua, name)
    if warped.check(GetConVar("warped_target"):GetString()) and GetConVar("warped_target"):GetString() ~= 'NONE' then
...
function warped.init()
    ...
    hook.Add("Think", HMenu, function()
        if input.IsKeyDown(KEY_INSERT) and not DermaMain:IsVisible() and not insertdown then
            insertdown = true
            DermaMain:Show()
            surface.PlaySound("UI/buttonclick.wav")
        elseif input.IsKeyDown(KEY_INSERT) and DermaMain:IsVisible() and not insertdown then
            insertdown = true
            DermaMain:Hide()
            surface.PlaySound("UI/buttonclick.wav")
